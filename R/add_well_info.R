...
#' 
#' @export
merge_plate_data <- function(d, well_info, plate_format = "plate_view", wells_to_ignore = c("EMPTY", "BW"), force_lower = TRUE) {
  ...
  if (plate_format == "plate_view") {
    ...
    well_info <- data.table::melt(well_info, id.vars = "row", variable.name = "col", value.name = "info")
  ...
  well_info <- well_info[!info %in% wells_to_ignore]
  data.table::setorder(well_info, well)
  data.table::setorder(d, well)
  d[well_info, on = c("well", "col", "row")]
}
